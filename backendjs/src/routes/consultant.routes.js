...
// POST /api/consultant/ask - Authentication optional (guest mode supported)
router.post('/ask', aiLimiter, async (req, res) => {
    try {
        ...
        // Get userId from token if available (guest mode if not)
        ...
        // Check and deduct credits (only for authenticated users)
        ...
        if (useAI) {
            ...
            // Call AI Service (DeepSeek, OpenAI, OpenRouter, etc.)
            answerData = await aiService.generateAnswer(
                baziContext,
                luckCyclesData,
                finalQuestionText,
                persona || 'huyen_co',
...
5. [Câu hỏi 5 - tùy chọn]`;
        ...
        // Call AI Service API
        const rawResponse = await aiService.generateCompletion(prompt, persona);
