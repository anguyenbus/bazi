/**
 * AI Service for BaZi Consultant
 * Supports multiple AI providers (DeepSeek, OpenAI, OpenRouter, Zhipu, etc.)
...
/**
 * Custom fetch using native https module for Zhipu
 * Works around undici connection issues
 * Includes DNS fallback using known IP addresses
 */
function nativeFetch(url, options) {
    return new Promise((resolve, reject) => {
        ...
        // Known IP fallbacks for Zhipu (in case DNS fails)
        const IP_FALLBACKS = {
            'bigmodel.cn': '60.205.172.105'
        };
        ...
        req.on('error', (err) => {
            // DNS error fallback - try with known IP
            if ((err.code === 'EAI_AGAIN' || err.code === 'ENOTFOUND') && IP_FALLBACKS[parsedUrl.hostname]) {
                console.log(`[nativeFetch] DNS error for ${parsedUrl.hostname}, using IP fallback`);
                reqOptions.hostname = IP_FALLBACKS[parsedUrl.hostname];
                reqOptions.rejectUnauthorized = false; // May need to skip cert verification for IP
                const retryReq = client.request(reqOptions, (res) => {
                    let data = '';
                    res.on('data', (chunk) => { data += chunk; });
                    res.on('end', () => {
                        resolve({
...
class AIService {
    constructor() {
        ...
        this.apiKey = process.env.AI_API_KEY || process.env.DEEPSEEK_API_KEY || process.env.OPENROUTER_API_KEY;
    ...
    getBaseUrl() {
        ...
        return urls[this.provider] || 'https://api.deepseek.com';
    }
    ...
    /**
     * Generate Zhipu AI JWT token from API key
     * Zhipu API key format: id.secret
     * JWT exp and timestamp must be in seconds, not milliseconds
     */
    generateZhipuToken() {
        ...
        const exp = now + 3600; // 1 hour expiration in seconds
    ...
    getAuthToken() {
        if (this.provider === 'zhipu') {
            return this.generateZhipuToken();
        }
        return this.apiKey;
    }
    ...
    getHeaders() {
        ...
        switch (this.provider) {
            ...
            case 'openrouter':
                headers['Authorization'] = `Bearer ${this.apiKey}`;
                headers['HTTP-Referer'] = 'https://huyencobattu.com';
                headers['X-Title'] = 'BaZi Consultant';
                break;
            case 'zhipu':
                // Zhipu uses Bearer token with generated JWT
    ...
    getModelName() {
        const models = {
            'deepseek': process.env.AI_MODEL || 'deepseek-chat',
            'openai': process.env.AI_MODEL || 'gpt-4o-mini',
            'openrouter': process.env.AI_MODEL || 'deepseek/deepseek-chat',
            'zhipu': process.env.AI_MODEL || 'glm-4-flash'
        };
    ...
    async generateAnswer(baziContext, luckCyclesData, questionText, personaId = 'huyen_co', partnerContext = null) {
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                console.log(`[AI:${this.provider}] Attempt ${attempt}/${this.maxRetries}...`);

                // Use native fetch for Zhipu to avoid undici connection issues
                const fetchFn = this.provider === 'zhipu' ? nativeFetch : fetch;

                const response = await fetchFn(this.getEndpoint(), {
                    method: 'POST',
                ...
                if (!response.ok) {
                    ...
                    throw new Error(`AI API error: ${response.status} - ${errorData.error?.message || errorData.message || 'Unknown error'}`);
                }

                const data = await response.json();
                // Zhipu GLM-5 returns content in reasoning_content field
                const message = data.choices?.[0]?.message || {};
                const content = message.content || message.reasoning_content || '';

                console.log(`[AI:${this.provider}] Success on attempt ${attempt}`);
                // Split response into paragraphs for frontend display
                return this.formatResponse(content);
            } catch (error) {
                lastError = error;
                console.error(`[AI:${this.provider}] Attempt ${attempt} failed:`, error.message);
                ...
                console.log(`[AI:${this.provider}] Waiting ${waitTime}ms before retry...`);
                await new Promise(resolve => setTimeout(resolve, waitTime));
            }
        }

        console.error(`[AI:${this.provider}] All attempts failed:`, lastError);
        // Return fallback response instead of throwing
        return this.getFallbackResponse(questionText, personaId);
    }
     ...
     */
    getFallbackResponse(questionText, personaId = 'huyen_co') {
        if (personaId === 'menh_meo') {
            return {
                answer: [
                    `Ối dồi ôi, server đang bận lắm nè con ơi!`,
                    `Thầy Mèo đang chill một chút, con thử lại sau nha! 😸`,
                    `Nhưng nhìn sơ qua lá số thì Thầy thấy con cũng vibe lắm đó, năng lượng dồi dào, tiềm năng phát triển cực mạnh.`
                ],
                followUps: [
...
## CÂU HỎI CỦA NGƯỜI DÙNG
 ...
 3. CUỐI CÙNG LÀ PHẦN [FOLLOW_UP] VỚI 3-5 CÂU HỎI GỢI MỞ.
    ...
    async generateCompletion(prompt, personaId = 'huyen_co') {
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                console.log(`[AI:${this.provider}/Completion] Attempt ${attempt}/${this.maxRetries}...`);
                ...
                console.log(`[AI:${this.provider}/Completion] Success on attempt ${attempt}`);
                ...
                lastError = error;
                console.error(`[AI:${this.provider}/Completion] Attempt ${attempt} failed:`, error.message);

                if (attempt < this.maxRetries && this.isRetryableError(error)) {
                    const delay = Math.pow(2, attempt) * 1000;
                    console.log(`[AI:${this.provider}/Completion] Retrying in ${delay}ms...`);
                    await new Promise(resolve => setTimeout(resolve, delay));
                }
            }
        }

        console.error(`[AI:${this.provider}/Completion] All attempts failed, returning fallback`);
    ...
    async generateMatchingAnswer(person1Ctx, person2Ctx, relationshipType = 'romance', personaId = 'huyen_co') {
        ...
        try {
            ...
            // Build request body - different providers have different parameters
            const requestBody = {
                model: this.getModelName(),
                messages: [
                    { role: 'system', content: systemPrompt },
                ...
                max_tokens: this.provider === 'zhipu' ? 8000 : 2000,
                temperature: 0.7
            };

            // Only add response_format for OpenAI-compatible providers that support it
            ...
            if (typeof controller !== 'undefined') clearTimeout(timeoutId);
            ...
            const data = await response.json();
            console.log(`[AI:${this.provider}/Matching] Raw response:`, JSON.stringify(data, null, 2).substring(0, 500));
            ...
            try {
                ...
                console.log(`[AI:${this.provider}/Matching] Successfully parsed and validated JSON response`);
                return parsedResult;
            } catch (jsonError) {
                console.error(`[AI:${this.provider}/Matching] JSON parsing failed:`, jsonError.message);
                console.error(`[AI:${this.provider}/Matching] Returning fallback response`);
            ...
            console.error(`[AI:${this.provider}/Matching] Fatal error:`, error);
